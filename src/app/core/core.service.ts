...
export class CoreService {
    ...
    public refreshAll() {
        this.getProductos.next(CacheService.get('productos', 'getProductos'));
        this.getSearch.next(this.search);
        this.getCartStatus.next({total: this.total, cantidad: this.cantidad});
    }

    public setProductos(productos) {
        this.getProductos.next(productos);
    }

    public setCategorias(categorias) {
        this.getCategorias.next(categorias);
    }

    public setLoginStatus(loginStatus) {
        this.getLoginStatus.next(loginStatus.showLogin);
    }
    ...
    public updateCarrito(item) {
        ...
        if (encontrado) {
            if (!item.en_carrito) {
                delete carrito[encontrado];
            } else {
                carrito[encontrado] = item.cantidad;
            }
        } else {
    ...
    calcularTotal() {
        ...
        let leng = productos.length;
        let cantidad = 0;

        while (leng--) {
            if (carrito[productos[leng]['producto_id']]) {
                total += (productos[leng].precios[0].precio) * carrito[productos[leng]['producto_id']];
                cantidad++;
            }
        }
    ...
    /// FILTERING
    public filterProducts(params, values, exact_match) {

        let productos = CacheService.get('productos', 'getProductos');

        return this.filter(params, values, exact_match, productos);
